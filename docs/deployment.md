...
TokenBel Wiki is deployed as a **Cloudflare Worker with Static Assets**. It is not a Cloudflare Pages project and has no KV namespace or origin server. The only Worker runtime code is `worker.js`: a single-purpose `Accept: text/markdown` content-negotiation script (zone-level Cloudflare "Markdown for Agents" would require a Pro+ plan, so negotiation is implemented in-repo).
...
The Markdown line must start with `# База знаний TokenBel`; the CSS request with the markdown `Accept` header must still return `text/css` (static assets are never rewritten). Also verify `Vary: Accept` on negotiated responses and that `/news/page/2/` with `Accept: text/markdown` falls back to `text/html`.
