...
- Новые изображения не коммитьте в leaf bundles. Храните их только в gitignored `.wiki-media/inbox/` и используйте image marker `![alt](upload:section/file.png)`; для имён с пробелами — `![alt](<upload:section/file name.png>)` (угловые скобки обрамляют URL внутри markdown-синтаксиса, не отдельный marker). `wiki-media publish` заменяет marker на content-addressed CDN URL после R2 verification; не-image ссылки с `upload:` запрещены. Контракт marker'а и R2-логика — в `tools/wiki-media/AGENTS.md` и `tools/wiki-media/README.md`.
