...
Вкладка «Позиции» — это главная часть страницы портфеля. Здесь собраны все ценные бумаги, токены и депозиты, которые вы держите в выбранном портфеле: сколько у вас единиц каждого инструмента, по какой цене, сколько они стоят сейчас и какой доход уже принесли.
...
- **Добавить операцию:** открывает окно для добавления покупки, продажи, погашения или операции по депозиту.
...
- **По типу инструмента:** позиции объединяются по типу (Токен, Облигация, Акция, Депозит). Помогает увидеть структуру портфеля по видам активов.
...
Это основная таблица вкладки. Каждая строка — одна позиция (токен, облигация, акция или депозит), которой вы владеете в этом портфеле. В режимах группировки вместо отдельных строк могут отображаться строки-группы со сводными суммами.
...
- **Инструмент:** название конкретного инструмента (токена, облигации, акции). Нажатие на название открывает страницу инструмента.
- **Тип инструмента:** вид актива — Токен, Облигация, Акция, Депозит или Ценная бумага.
- **Валюта:** валюта, в которой номинирован инструмент (BYN, EUR, USD, RUB).
- **Количество:** сколько единиц инструмента у вас сейчас в этой позиции.
- **Номинальная цена:** номинал одной единицы инструмента в её исходной валюте.
...
- **Сумма:** стоимость позиции в исходной валюте (количество × текущая цена).
...
- **Удалить (значок корзины):** удаляет позицию целиком вместе со всей историей операций. Перед удалением система попросит подтвердить действие.
...
Ниже основной таблицы находится отдельный раздел для неактивных позиций. Сюда попадают инструменты, у которых количество равно нулю, либо эмиссия которых уже завершена (истёк срок обращения).
...
- **Тип инструмента:** вид актива — Токен, Облигация, Акция, Депозит.
- **Валюта:** валюта инструмента.
- **Количество:** сколько единиц осталось (обычно ноль).
...
- **Действия:** кнопка «Удалить» (значок корзины) для полного удаления позиции вместе с историей операций.
...
Открывается кнопкой «Добавить операцию» и позволяет внести новую операцию по покупке, продаже, погашению или пополнению.

#### Выбор типа инструмента

Вверху окна находится переключатель из четырёх вкладок: **Токены, Облигации, Акции, Депозиты**. Выбор вкладки меняет состав полей формы.
...
Открывается кнопкой «История операций» (значок часов) в колонке «Действия». Показывает все операции по выбранной позиции.
...
- **Позиция:** отдельный инструмент в портфеле с текущим количеством и историей операций по нему.
- **Инструмент:** ценная бумага, токен или депозит, которым вы владеете или совершаете операции.
...
- **Номинальная цена:** стоимость одной единицы инструмента, установленная при выпуске.
