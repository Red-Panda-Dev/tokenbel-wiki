...
Статическая русскоязычная база знаний TokenBel, собираемая Hugo и публикуемая на `https://wiki.tokenbel.info/` как Cloudflare Worker со Static Assets с runtime-скриптом `worker.js` (content negotiation `Accept: text/markdown` и обогащение агентного обнаружения: Link-заголовки главной, канонические content-type discovery-документов, каталог API `/.well-known/api-catalog`; без KV/bindings, кроме `ASSETS`). Внешних Hugo-тем нет — вся вёрстка локальная.
...
- Worker имеет ровно две runtime-обязанности: (1) content negotiation — запросы с `Accept: text/markdown` получают Hugo-generated `index.md` (output format `Markdown` в `hugo.yaml`), остальное проходит в статику без изменений; нет md-варианта (404, `/page/N/`) — прозрачный фолбэк на HTML; (2) обогащение агентного обнаружения — Link-заголовки главной (RFC 8288/9727): `describedby` на `/llms.txt` и `/sitemap.xml`, и явный `charset=utf-8` у discovery-документов `/llms.txt` и `/auth.md` (Static Assets отдаёт `text/markdown` и `text/plain` без charset — клиенты неверно декодируют кириллицу; файл `_headers` не применяется к ответам, прошедшим через Worker при `run_worker_first`) и `application/linkset+json` у каталога API `/.well-known/api-catalog` (RFC 9727; у файла нет расширения, поэтому Static Assets не определяет тип) — проверяет `tests/check_link_headers.mjs`. Relation types из RFC 8631 применяются только внутри каталога; Link-заголовки главной остаются ровно двумя `describedby`. Новая runtime-логика — только по отдельному архитектурному решению.
- Агентные discovery-документы лежат в `static/` и публикуются дословно: `/llms.txt` (описание базы), `/auth.md` (правила доступа по спецификации auth.md — самодостаточный документ: только анонимный доступ, без регистрации и OAuth-метаданных), `/.well-known/api-catalog` (каталог API по RFC 9727: единственный API — фактический read-only HTTP-интерфейс вики; `service-desc` → `/openapi.json`, `service-doc` → `/llms.txt`, `service-meta` → `/auth.md`; relation `status` намеренно отсутствует — health-эндпоинта нет) и `/openapi.json` (OpenAPI 3.1, описание фактического поведения: анонимный read-only + markdown-неготиация). H1 в `auth.md` обязан содержать `auth.md` — проверяет `make check`.
